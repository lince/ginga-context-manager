...
#include "recommender/utils/Utils.h"^M
...
public:
	...
	//Codigo de depuração - Ângelo Bitar
	/** 
	 * Converter ints to strings
	 * @param int number the number you want convert
	 * @return string a string of the number you want convert
	 */
	string convInt2String(int number);
	/** 
	 * Converter floats to strings
	 * @param float number the number you want convert
	 * @return string a string of the number you want convert
	 */
	string convFloat2String(float number);
	/** 
	 * Verify if is necessary send new menssagem to profile monitor
	 * @return true if  have change informations
	 */
	bool isNewData(){
		return newData;
	};
	 ...
	 */	
	void candidatesClear();
	/** 
	 * Change the flag new data
	 * @param bool data indicate true ou false to flag
	 */	
	...
	/** 
	 * Create a thread named recomenda
	 */	
	void checkRecommenderModule();

	/** 
	 * Get the thread that was create in chekRecommenderModule
	 * @retunr pthread_t thread recommenda
	 */	
	pthread_t getThread();
	
	/** 
	 * Flag used to stop the thread
	 * @retunr bool false if you want stop the thread
	 */	
