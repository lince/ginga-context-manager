...
namespace tv {
	ProgramAVPlayerAdapter::ProgramAVPlayerAdapter(
		...
		pav = ((ProgramHandlerCreator*)(cm->getObject(playerCompName)))();
#else
		pav = ProgramAV::getInstance();
#endif
	...
	bool ProgramAVPlayerAdapter::stop() {
		pav->setPropertyValue("bounds", "");     // full screen
		pav->setPropertyValue("soundLevel", ""); // full sound
	...
	void ProgramAVPlayerAdapter::createPlayer() {
		...
		if (descriptor != NULL) {
			...
			pav->setPropertyValue("soundLevel", soundLevel);
			updateAVBounds();
		}
	...
	bool ProgramAVPlayerAdapter::setPropertyValue(
		...
		cout << "ProgramAVPlayerAdapter::setPropertyValue '";
		cout << propName << "' ";
		cout << value << "'" << endl;
		...
		if (propName == "size" || propName == "location" ||
			...
			if (value == "true") {
				pav->setPropertyValue("bounds", ""); // fullscreen
			...
			pav->setPropertyValue(propName, value);
		}
	...
	void ProgramAVPlayerAdapter::updateAVBounds() {
		...
		if (object != NULL && object->getDescriptor() != NULL) {
			...
			cout << "ProgramAVPlayerAdapter::updateAVBounds Warning!";
			cout << "NULL object or descriptor" << endl;
		}
	}
}
}
...
extern "C" ::br::pucrio::telemidia::ginga::ncl::adapters::IPlayerAdapter*
	...
	return new ::br::pucrio::telemidia::ginga::ncl::adapters::av::tv::
			ProgramAVPlayerAdapter(manager);
}
