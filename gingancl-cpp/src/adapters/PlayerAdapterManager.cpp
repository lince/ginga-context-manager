...
cout << "-----------------before readConfigFiles" << endl;
		readConfigFiles();

		running = true;
cout << "-----------------before have_wac" << endl;
//#IF HAVE_WAC
		IComponentManager* cm = IComponentManager::getCMInstance();
		stateManager = ((StateManagerCreator*)(
					cm->getObject("StateManager")))();
//#ENDIF
cout << "-----------------after have_wac" << endl;
		start();
	}
	...
	void PlayerAdapterManager::readConfigFiles() {
		...
		fisMime.open(
				"/usr/local/etc/ginga/files/cfg/formatter/mimedefs.ini",
				ifstream::in);
		...
		fisCtrl.open("/usr/local/etc/ginga/files/cfg/formatter/ctrldefs.ini");
		if (!fisCtrl.is_open()) {
...
//#IF HAVE_WAC
		stateManager->addStateProvider(objId, player);
//#ENDIF
