...
namespace adapters {
	...
	FormatterPlayerAdapter::~FormatterPlayerAdapter() {
		...
		if (playerCompName != "" && playerCompName != "ProgramAV") {
			printTimeStamp();
			cout << "FormatterPlayerAdapter::~FormatterPlayerAdapter ";
			cout << "release '" << playerCompName << "'" << endl;
	...
	void FormatterPlayerAdapter::createPlayer() {
		...
		cout << "FormatterPlayerAdapter::createPlayer for '" << mrl;
		cout << "'" << endl;

		if (player == NULL) {
#if HAVE_COMPSUPPORT
			playerCompName = "Player";
			player = ((PlayerCreator*)(cm->getObject("Player")))(NULL, false);
#else
		...
		if (object != NULL) {
			...
			if (object != NULL && !object->start() && player != NULL) {
				...
				if (nextTransition != NULL &&
					...
					cout << "FormatterPlayerAdapter::start ";
					cout << "creating TIME NominalEventMonitor for '";
					cout << object->getId() << "'";
					cout << endl;
