...
//TESTE WAC
#include <ginga/ncl/generator/INclGenerator.h>
...
namespace ncl {
	...
	FormatterMediator::FormatterMediator(NclPlayerData* pData) : Player("") {
		...
		cout << "FormatterMediator::FormatterMediator(" << this << ")";
		cout << " isEmbedded = '" << isEmbedded << "'" << endl;
	}
	...
	FormatterMediator::~FormatterMediator() {
		...
		cout << "FormatterMediator::~FormatterMediator '";
		cout << data->playerId << "'";
		cout << endl;
	...
	void FormatterMediator::printData(NclPlayerData* data) {
		...
		cout << " devClass: " << data->devClass;
		cout << " x: " << data->x;
		cout << " y: " << data->y;
		cout << " w: " << data->w;
		cout << " h: " << data->h;
		cout << " enableGfx: " << data->enableGfx;
	...
	bool FormatterMediator::stopDocument(string documentId) {
		...
		//TESTE GINGA WAC
		INclGenerator* nclGenerator = NULL;
		if (cm != NULL) {
			nclGenerator = ((NclGeneratorCreator*)(cm->getObject("NclGenerator")))();
			if (nclGenerator != NULL) {
				string fileName = "/tmp/nclGerado.ncl";
				cerr<<"Vamos tentar gerar o arquivo"<<nclGenerator->generateNclFile(
						fileName, currentDocument)<<endl<<endl<<endl;
			} else {
				cerr<<"A geração não deu certo."<<endl<<endl<<endl;
			}
		}
		//TESTE GINGA WAC
			...
			if (src != "") {
				if (targetDev == 0) {
					...
					if (descriptor != NULL &&
						...
						cout << "FormatterMediator::getDepUriFromNode ";
						cout << "checking descriptor '";
						cout << ((Descriptor*)descriptor)->getId();
						cout << "'" << endl;
...
//#IF HAVE_WAC
	PlayerState* FormatterMediator::getPlayerState() {
