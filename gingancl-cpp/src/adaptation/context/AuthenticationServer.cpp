...
void AuthenticationServer::saveUser(int uid){
	ictxm = ImplicitContextManager::getInstance();
	lagt = LocalAgent::getInstance();
	stringstream s;
	stringstream ss;
	stringstream sss;
		if (ictxm != NULL && ctxm != NULL&& lagt != NULL) {
			pctx->lock();
			IGingaUser *u = ctxm->getUser(uid);
			s << ctxm->getCurrentUserId();
			ss << u->getUserGenre();
			sss << u->getUserAge();
			lagt->setNewUser(s.str().c_str(),ss.str().c_str(), u->getUserLocation(), sss.str().c_str());
			pctx->unlock();
		}
};
...
const char *AuthenticationServer::login(int csocket, string userid, string passwd) {
	...
	if (u != NULL) {
		...
		//cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@Apos instancia no ICTXM" << endl;
		saveUser(uid);
