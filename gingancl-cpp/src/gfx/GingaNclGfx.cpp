...
#include "../../../gingacc-system/include/io/interface/content/text/DFBFontProvider.h"
...
namespace ncl {
	...
	void GingaNclGfx::show(int l, int t, int w, int h) {
		...
		s = ((ImageRenderer*)(cm->getObject("ImageRenderer")))(
				(char*)("/usr/local/etc/ginga/files/img/roller/loading.png"));
#else
		bg = new DFBWindow(l, t, w, h);
		s = ImagePlayer::renderImage(
				"/usr/local/etc/ginga/files/img/roller/loading.png");
#endif
	...
	void GingaNclGfx::showText(IWindow* window, string text) {
		...
		fontUri = "/usr/local/etc/ginga/files/font/decker.ttf";
		fontSize = 12;
